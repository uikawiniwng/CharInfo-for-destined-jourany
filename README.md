...
> 这是一份**项目专用 README**，不是模板说明
>
> 目标是让新协作者和 AI 在不口头交接的情况下，快速理解这个仓库当前在做什么、核心模块各自负责什么、该从哪里改
...
这个项目不是在做通用后台系统，而是在做**酒馆助手场景中的角色信息展示前端**

- 输入：YAML 角色文本
- 输出：可视化角色卡页面 + 可选导入动作
- 重点：视觉表现、容错解析、可维护的数据映射

如果你在继续开发，请优先围绕 `3.0` 目录，不要把新逻辑分散到旧版本文件
...
```bash
git commit -m "fix: 调整桌面端属性旗帜数字位置"
git commit -m "style: 恢复技能装备物品的品质颜色"
```
...
如果你的仓库启用了 Actions，可继续用 CI 做自动构建
...
- 先读本 README 再改代码
- 不确定需求时，优先确认“这个改动属于显示层、解析层还是导入层”
- 改动请保持最小影响面，避免把多个目标混在同一个提交
